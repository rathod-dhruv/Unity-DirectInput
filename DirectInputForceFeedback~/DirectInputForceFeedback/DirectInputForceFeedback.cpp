...
std::map<DeviceGUID, LPDIRECTINPUTEFFECT> _ActiveEffects; // Handle to Start/Stop Effect
...
//CUSTOM FUNCTIONS - DHRUV -- START

//***********************************************************
//////////////////////////////////////////////////////////////

HRESULT CreatePeriodicFFBEffect(LPCSTR guidInstance, Effects::Type effectType) {
    ...
    effect.dwSize = sizeof(DIEFFECT);
    effect.dwFlags = DIEFF_CARTESIAN | DIEFF_OBJECTOFFSETS;
    effect.dwDuration = INFINITE; // Infinite duration
    effect.dwSamplePeriod = 0;
    effect.dwGain = DI_FFNOMINALMAX;
    ...
    effect.lpvTypeSpecificParams = &pe;
    effect.dwStartDelay = 0;

    LPDIRECTINPUTEFFECT diEffect;
    GUID guidEffect =  GUID_Square;

    HRESULT hr = device->CreateEffect(guidEffect, &effect, &diEffect, nullptr);
    if (FAILED(hr)) return hr;

    hr = diEffect->Start(1, 0); // Start the effect
    if (FAILED(hr)) return hr;

    _ActiveEffects[GUIDString] = diEffect; // Store the effect in a map for later use

    return S_OK;
}
HRESULT UpdatePeriodicFFBEffect(LPCSTR guidInstance, Effects::Type effectType, int magnitude, int duration) {
    std::string GUIDString((LPCSTR)guidInstance);
    LPDIRECTINPUTDEVICE8 device = _ActiveDevices[GUIDString];
    LPDIRECTINPUTEFFECT diEffect = _ActiveEffects[GUIDString];

    if (device == nullptr || diEffect == nullptr) return E_FAIL;

    DIPERIODIC pe = { magnitude, 0, 1000, 20000 }; // Update the magnitude

    DIEFFECT effect;
    ZeroMemory(&effect, sizeof(effect));
    effect.dwSize = sizeof(DIEFFECT);
    effect.dwFlags = DIEFF_CARTESIAN | DIEFF_OBJECTOFFSETS;
    effect.dwDuration = (duration > 0) ? static_cast<DWORD>(duration * DI_SECONDS) : INFINITE;
    effect.dwSamplePeriod = 0;
    effect.dwGain = DI_FFNOMINALMAX;
    effect.dwTriggerButton = DIEB_NOTRIGGER;
    effect.dwTriggerRepeatInterval = 0;
    ...
    HRESULT hr = diEffect->SetParameters(&effect, DIEP_DIRECTION | DIEP_TYPESPECIFICPARAMS | DIEP_DURATION | DIEP_START);
    if (FAILED(hr)) return hr;

    return S_OK;
}

HRESULT DestroyPeriodicFFBEffect(LPCSTR guidInstance) {
    std::string GUIDString((LPCSTR)guidInstance);
    LPDIRECTINPUTEFFECT diEffect = _ActiveEffects[GUIDString];

    if (diEffect == nullptr) return E_FAIL;

    HRESULT hr = diEffect->Stop(); // Stop the effect
    if (FAILED(hr)) return hr;

    diEffect->Release(); // Release the effect
    _ActiveEffects.erase(GUIDString); // Remove it from the map
