...
public struct FlatJoyState2State : IInputStateTypeInfo
{
    ...
    public UInt64 buttonsA; // Buttons separated into banks of 64-Bits to fit into Unsigned 64-bit integer
    ...
    public UInt64 buttonsB; // Buttons separated into banks of 64-Bits to fit into Unsigned 64-bit integer
    ...
    // Store each DPAD in chunks of 4 bits inside a 16-bit Unsigned integer   

    ///////////////// ----------  START :::  CHANGES BY DHRUV RATHOD -------------------/////////////////


    /////////////////*------- REASON :: VR - OpenXR -- It was throwing errors becasue OpenXR also do the D-Pad binding and 
    ///Below code were also trying to do it so it was creating issue for my usecase, so I commented it, you can try to find better solution if you can *//////////////////
    ...
    ///////////////// ----------  END :::  CHANGES BY DHRUV RATHOD -------------------/////////////////

    public UInt16 rgdwPOV;
}
...
public class DirectInputDevice : InputDevice, IInputUpdateCallbackReceiver
{
    ...
    {
        Initialize(); // Ensure layouts are ready even when just in the editor
    }
#endif
    ...
    public static void GenerateISLayouts()
    {
        foreach (DeviceInfo device in DIManager.devices)
        {
            try
            {
                Debug.Log("Debug For DI :: " + device.productName);
            ...
            {
                Debug.Log("Exception For DI :: " + device.productName);
    ...
    {
        if (DIManager.Attach(this.description.serial))
        { // Doing this in FinishSetup allows events to be re-created on Domain reloads
    ...
    {
        if (InputSystem.devices.OfType<DirectInputDevice>().Where(d => d.description.serial == device.guidInstance).Any())
        {
            return true; // Device already exists
        }
        if (DIManager.Attach(device))
        { // Attach to DirectInput device, proceed if successfully connected
